...
class Word2Box(BaseModule):
	...
	def embedding_similarity(self, b1, b2):

		if self.intersection_temp == 0.0:
			score = b1.intersection_log_soft_volume(
				b2, temp=self.volume_temp
			)
		else:
			score = b1.gumbel_intersection_log_volume(
				b2,
				volume_temp=self.volume_temp,
				intersection_temp=self.intersection_temp,
			)
	...
	#takes in an array of word indices and returns an averaged box embedding
	def sentence_embeddings(self, word_idx):
		with torch.no_grad():
			word_embeddings = self.embeddings_word(torch.tensor(word_idx, device=device))

			#compute the mean of z and Z, stack into a new tensor, and use [None] to change
			#the shape from [2, d], to [1, 2, d]
			avg=BoxTensor(torch.stack((
				torch.mean(word_embeddings.z, dim=0),
				torch.mean(word_embeddings.Z, dim=0)
			))[None])
			
		return avg

	#concatenates z and Z vectors into a single 2n dimensional vector for each word
	def vec_embeddings(self):
		w=self.embeddings_word.weight
