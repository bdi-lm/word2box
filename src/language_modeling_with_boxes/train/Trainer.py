...
class TrainerWordSimilarity(Trainer):
	...
	def train_model(
		...
		hparams={
			...
			'lr':self.lr,
			'window_size':model.n_gram,
			'negative_samples':self.negative_samples,
			'subsampling_threshold':self.subsample_thresh,
		}
		self.writer.add_hparams(hparams, {'placeholder':0})
		self.writer.flush()
		...
		for epoch in tqdm(range(num_epochs)):
			...
			for i, batch in enumerate(tqdm(self.train_iter)):
				...
				if i % int(len(self.train_iter) / self.log_frequency) == 0:
					...
					#log to tensorboard
					for key in ws_metric.keys():
						self.writer.add_scalar('metric/'+key, ws_metric[key], step)
					self.writer.add_scalar('Loss', epoch_loss[-1], step)
					...
					model.train()
				#use steps to log instead of epoch
				step+=1
	...
	def fuzzy_jaccard(self, w1, w2):
		intersection = (
			torch.sum(torch.minimum(w1.z, w2.z)) + 
			torch.sum(torch.minimum(w1.Z, w2.Z))
		)

		union = (
			torch.sum(torch.maximum(w1.z, w2.z)) +
			torch.sum(torch.maximum(w1.Z, w2.Z))
		)
				...
				with xopen(os.path.join(self.similarity_datasets_dir, file)) as f:
					...
					for row in reader:
						...
						if (
							...
							fj_score = self.fuzzy_jaccard(
								model.embeddings_word(word1), 
								model.embeddings_word(word2)
							)
							if file.title() == "Hyperlex-Dev.Txt":
								score = model.conditional_similarity(word1, word2)

							predicted_scores.append(score.item())
							predicted_fj_scores.append(fj_score)
